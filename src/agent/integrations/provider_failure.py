...
class PostgresProviderQueueFailureCoordinator:
    ...
    async def _move_once(
        ...
        confirmation_key = f"provider-queue-failure:{operation_id}:confirmed:{request_id}"
        async with self._pool.connection() as connection:
            async with connection.transaction():
                async with connection.cursor(row_factory=dict_row) as cursor:
                    ...
                    if operation.tenant_id != scope.tenant_id or operation.customer_id != scope.customer_id:
                        raise OperationNotFoundError(str(operation_id))
                    await cursor.execute(
                    ...
                    if operation.status == "manual_review" and operation.support_case_id is not None:
                        await cursor.execute(f"SELECT {_CASE_COLUMNS} FROM case_management.support_cases WHERE case_id = %s", (operation.support_case_id,))
                        case_row = await cursor.fetchone()
                        if case_row is None:
                            raise ValueError("operation manual-review case is unavailable")
                        ...
                        if (
                            ...
                            raise ValueError("operation manual-review case association is invalid")
                        return ProviderQueueFailureResult(operation, case, "duplicate_ignored")
                    if operation.status != "pending_confirmation":
                        raise ValueError("operation is not eligible for provider manual review")
                    await cursor.execute(
                    ...
                    if case_row is None:
                        ...
                        case_event = SupportCaseEvent(event_id=uuid4(), idempotency_key=f"provider-queue-failure:{operation_id}:case:{request_id}", case_id=case.case_id, event_type="case_created", source_message_id=case.source_message_id, order_id=case.order_id, reason_codes=case.reason_codes, triggering_message_excerpt=case.triggering_message_excerpt, current_priority="p1", current_status="open", customer_id=case.customer_id, tenant_id=case.tenant_id, created_at=now)
                        await cursor.execute(f"INSERT INTO case_management.support_cases ({_CASE_COLUMNS}) VALUES ({', '.join(['%s'] * 20)})", _case_values(case))
                        await cursor.execute(f"INSERT INTO case_management.support_case_events ({CASE_EVENT_COLUMNS}) VALUES ({', '.join(['%s'] * 24)})", case_event_values(case_event))
                        action = "created"
                    else:
                        existing = _case_from_row(case_row)
                        reasons = tuple(dict.fromkeys((*existing.reason_codes, "provider_delivery_failed")))
                        case = existing.model_copy(update={"priority":"p1", "reason_codes":reasons, "updated_at":now, "version":existing.version + 1})
                        await cursor.execute("UPDATE case_management.support_cases SET priority=%s, reason_codes=%s, updated_at=%s, version=%s WHERE case_id=%s AND version=%s", (case.priority, list(case.reason_codes), now, case.version, case.case_id, existing.version))
                        if cursor.rowcount != 1:
                            raise ConcurrentOperationUpdateError(str(operation.operation_id))
                        case_event = SupportCaseEvent(event_id=uuid4(), idempotency_key=f"provider-queue-failure:{operation_id}:case:{request_id}", case_id=case.case_id, event_type="trigger_appended", source_message_id=operation.source_message_id, order_id=operation.order_id, reason_codes=("provider_delivery_failed",), triggering_message_excerpt=operation.request_excerpt, previous_priority=existing.priority, current_priority=case.priority, current_status=case.status, actor="system", customer_id=case.customer_id, tenant_id=case.tenant_id, created_at=now)
                        await cursor.execute(f"INSERT INTO case_management.support_case_events ({CASE_EVENT_COLUMNS}) VALUES ({', '.join(['%s'] * 24)})", case_event_values(case_event))
                        action = "reused"
                    ...
                    await cursor.execute("UPDATE case_management.order_operations SET requires_manual_review=%s, review_case_type=%s, review_priority=%s, support_case_id=%s, status=%s, updated_at=%s, version=%s WHERE operation_id=%s AND version=%s", (True, "order_operation_review", "p1", case.case_id, "manual_review", now, updated.version, operation.operation_id, operation.version))
                    if cursor.rowcount != 1:
                        raise ConcurrentOperationUpdateError(str(operation.operation_id))
                    if not replay:
                        events = (
                            OrderOperationEvent(event_id=uuid4(), idempotency_key=confirmation_key, operation_id=operation.operation_id, event_type="confirmation_recorded", actor=scope.identity, customer_id=operation.customer_id, tenant_id=operation.tenant_id, created_at=now),
                            OrderOperationEvent(event_id=uuid4(), idempotency_key=f"provider-queue-failure:{operation_id}:status:{request_id}", operation_id=operation.operation_id, event_type="status_changed", previous_status=operation.status, current_status="manual_review", actor="system", customer_id=operation.customer_id, tenant_id=operation.tenant_id, created_at=now),
                            OrderOperationEvent(event_id=uuid4(), idempotency_key=f"provider-queue-failure:{operation_id}:case-attached:{request_id}", operation_id=operation.operation_id, event_type="support_case_attached", support_case_id=case.case_id, actor="system", customer_id=operation.customer_id, tenant_id=operation.tenant_id, created_at=now),
                        )
                        for event in events:
                            await cursor.execute(f"INSERT INTO case_management.order_operation_events ({OP_EVENT_COLUMNS}) VALUES ({', '.join(['%s'] * 12)})", _event_values(event))
